...
link_verilator: ## Generate simulation executable using Verilator
	...
	verilator $(VERILATOR_FLAGS) --no-timing --cc $(TOP_MODULE).v -y $(VERILOGDIR) --exe
...
release-verilog-artifacts: generate_verilog generate_boot_files link_verilator
	...
	@cp ${BSVOUTDIR}/out verilog-artifacts/sim/eclass
	@cp ${TOOLS_DIR}/shakti-tools/IITM_LICENSE.txt verilog-artifacts/LICENSE.txt
